...
namespace Magma
{
	VulkanIndexBuffer::VulkanIndexBuffer(const Ref<RenderDevice>& device, u32 count, const void* indices, const IndexSize& indexSize)
		: IndexBuffer(indexSize)
	{
		...
		VkDeviceSize memorySize = count * SizeofIndex(m_IndexSize);
		...
		memcpy(bufferData, indices, memorySize);
		vkUnmapMemory(m_Device, stagingBufferMemory);

		CreateBuffer(physicalDevice, m_Device, memorySize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
